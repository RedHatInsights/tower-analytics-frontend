...
export function PageActions<T extends object>(props: {
  ...
  const collapseButtons =
    ...
    (props.collapse === 'always' || !collapseBreakpoint);
  ...
  return (
    <Split
      hasGutter={
        (!iconOnly && selectedItem !== undefined) ||
...
function isPinnedAction<T extends object>(action: IPageAction<T>) {
  ...
  return (
    action.type !== PageActionType.seperator &&
...
export function isHiddenAction<T extends object>(
  ...
  switch (action.type) {
    ...
    case PageActionType.dropdown:
      return action.isHidden !== undefined && selectedItem
        ? action.isHidden(selectedItem)
        : false;
