...
const TemplatesList = ({ templates, isLoading, qp, title, jobType }) => {
  ...
  const displayJobType = type => {
    return (type === 'job' ? 'template' : 'workflow');
  }
  ...
  return (
    <>
      ...
      >
        ...
        {isLoading && (
          <PFDataListItem
            aria-labelledby={`${displayJobType(jobType)}s-loading`}
            key={isLoading}
            style={{ border: 'none' }}
          >
            <PFDataListCell>
        ...
        {!isLoading && templates.length <= 0 && (
          <PFDataListItem
            aria-labelledby={`${displayJobType(jobType)}s-no-data`}
            key={isLoading}
            style={{ border: 'none' }}
          >
            <PFDataListCell>
        ...
        {!isLoading &&
          templates.map(({ name, total_count, id }, index) => (
            <DataListItem aria-labelledby={`top-${displayJobType(jobType)}s-detail`} key={index}>
              <DataListCell>
                <a
                  onClick={() => {
                    setIsModalOpen(true);
