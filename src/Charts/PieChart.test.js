...
describe("Charts/PieChart/Tooltip", () => {
  ...
  it("should render data with success_rate successfully", () => {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      let rectangle = document.getElementById(
        "svg-chart-Tooltip.base-" + thisid1
      );
      ...
      let expectedRate =
        "(" + dataWithSuccess[i].success_rate + "% successful)";
  ...
  it("should render data without success_rate successfully", () => {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      paths[i].dispatchEvent(mouseover);
      let rectangle = document.getElementById(
        "svg-chart-Tooltip.base-" + thisid2
      );

      expect(rectangle.children.length).toBe(4);
      expect(rectangle.children[2].textContent.trim()).toBe(
        dataWithoutSuccess[i].name
      );
